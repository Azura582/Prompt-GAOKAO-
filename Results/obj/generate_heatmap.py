...
# 定义领域和对应的中文名称
DOMAINS = {
    'Commonsense_and_WorldKnowledge': '常识知识',
    'Data_and_StatisticalLiteracy': '数据统计',
    'Lang_Comp_and_Produc': '阅读理解',
    'Logical_Reasoning': '逻辑推理',
    'Mathematical_Reasoning': '数学推理',
    'Natural_Science': '自然科学',
    'Sociocultural_Understanding': '社会文化理解'
}
...
def read_scoring_rates(base_path='./'):
    """
    读取所有领域的得分率数据
    
    Args:
        base_path: Results/obj/ 目录的路径
    ...
    for domain_en, domain_cn in DOMAINS.items():
        csv_path = base_path / domain_en / 'scoring_rates.csv'
        if csv_path.exists():
            df = pd.read_csv(csv_path)
            # 提取策略和得分率
            for _, row in df.iterrows():
                strategy = row['strategy']
                scoring_rate = row['scoring_rate']
                
                if strategy not in scoring_data:
                    scoring_data[strategy] = {}
                scoring_data[strategy][domain_cn] = scoring_rate
        else:
    ...
    df_scoring.index = [STRATEGY_NAMES.get(idx, idx) for idx in df_scoring.index]
...
def plot_heatmap(df_scoring, best_domains, output_path='heatmap_strategy_domain.png'):
    """
    绘制热力图
